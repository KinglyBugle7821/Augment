...
public class TitaniumFusionForgeBlockEntityRenderer implements BlockEntityRenderer<TitaniumFusionForgeBlockEntity> {
    public TitaniumFusionForgeBlockEntityRenderer(BlockEntityRendererProvider.Context context){

    }

    @Override
    public void render(TitaniumFusionForgeBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {
        ItemRenderer itemRenderer = Minecraft.getInstance().getItemRenderer();

        ItemStack inputItemStack = pBlockEntity.itemHandler.getStackInSlot(TitaniumFusionForgeBlockEntity.INPUT_SLOT);
        renderSlot(pPoseStack, itemRenderer, inputItemStack, 0.5f, 0.7f, 0.5f, pPackedLight, pBuffer, pBlockEntity);

        ItemStack extraItemStack = pBlockEntity.itemHandler.getStackInSlot(TitaniumFusionForgeBlockEntity.EXTRA_SLOT);
        renderSlot(pPoseStack, itemRenderer, extraItemStack, 0.5f, 0.75f, 0.5f, pPackedLight, pBuffer, pBlockEntity);

        ItemStack outputItemStack = pBlockEntity.itemHandler.getStackInSlot(TitaniumFusionForgeBlockEntity.OUTPUT_SLOT);
