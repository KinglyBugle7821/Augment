...
public class SimpleFusionForgeBlockEntityRenderer implements BlockEntityRenderer<SimpleFusionForgeBlockEntity> {
    public SimpleFusionForgeBlockEntityRenderer(BlockEntityRendererProvider.Context context){

    }

    @Override
    public void render(SimpleFusionForgeBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {
        ItemRenderer itemRenderer = Minecraft.getInstance().getItemRenderer();

        ItemStack inputItemStack = pBlockEntity.itemHandler.getStackInSlot(SimpleFusionForgeBlockEntity.INPUT_SLOT);
        renderSlot(pPoseStack, itemRenderer, inputItemStack, 0.5f, 0.7f, 0.5f, pPackedLight, pBuffer, pBlockEntity);

        ItemStack extraItemStack = pBlockEntity.itemHandler.getStackInSlot(SimpleFusionForgeBlockEntity.EXTRA_SLOT);
        renderSlot(pPoseStack, itemRenderer, extraItemStack, 0.5f, 0.75f, 0.5f, pPackedLight, pBuffer, pBlockEntity);

        ItemStack outputItemStack = pBlockEntity.itemHandler.getStackInSlot(SimpleFusionForgeBlockEntity.OUTPUT_SLOT);
